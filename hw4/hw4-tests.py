...
else: check('subst', interpret.subst, [\
...
else: check(('unifying... ', ' ...'), lambda s1,s2: interpret.unify(parser(grammar, 'expression')(s1), parser(grammar, 'expression')(s2)), [\
    (["x", "5"], {"x":{"Number":[5]}}),\
...
try: interpret.evaluate
except: print("The evaluate() function is not defined.\n")
else: check(('evaluating... ', ' ...'), (lambda d,e: interpret.evaluate(interpret.build({}, parser(grammar, 'declaration')(d)), {}, parser(grammar, 'expression')(e))), [\
